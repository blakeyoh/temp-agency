# Temp Agency — Roster Index

Full index of the target 24-specialist roster (15 built, 9 planned — see
`docs/plan-v2.md` for the v2 refocus rationale). Load this file when browsing for the
right lead or lens. `✓ Built` entries have a corresponding `roster/[slug].md` file and
can be loaded now; `○ Planned` entries are tracked as GitHub issues and not yet
loadable.

-----

## Built (15)

|Slug                              |Title                            |Status  |Epistemological Lens                                                                                        |
|-----------------------------------|----------------------------------|--------|-------------------------------------------------------------------------------------------------------------|
|`systems-thinker`                  |Systems Thinker                   |✓ Built |Sees feedback loops, leverage points, and unintended consequences; asks "what does this system want to do?"  |
|`investigative-journalist`         |Investigative Journalist          |✓ Built |Follows incentives, challenges narratives, asks "who benefits from this story?"                              |
|`behavioral-psychologist`          |Behavioral Psychologist           |✓ Built |Reads behavior as data; surfaces the gap between what people say and do                                      |
|`executive-coach`                  |Executive Coach                   |✓ Built |Focuses on the person behind the decision; surfaces beliefs and blind spots                                  |
|`chief-of-staff`                   |Chief of Staff                    |✓ Built |Asks "what does the principal actually need?" vs. what they asked for                                        |
|`contrarian`                       |Contrarian                        |✓ Built |Devil's advocate and pre-mortem specialist; default lens when no high-contrast match exists                  |
|`left-fielder`                     |Left-Fielder                      |✓ Built |Imports a randomly selected domain's frameworks; creative forcing function                                   |
|`missiologist`                     |Missiologist                      |✓ Built |Asks what must be preserved vs. what is cultural wrapper as ideas travel to new contexts                     |
|`anthropologist`                   |Anthropologist                    |✓ Built |Observes without judgment; surfaces what people *do* vs. what they *say*                                     |
|`farmer`                           |Farmer                            |✓ Built |Embodied, patient knowledge from working systems you can't fully control; asks "what does this soil/season actually require?" |
|`civil-rights-activist`            |Civil Rights Activist             |✓ Built |Power analysis before strategy; moral urgency with tactical patience; asks "whose interests are centered and who bears the cost?" |
|`franciscan-monk`                  |Franciscan Monk                   |✓ Built |Simplicity as epistemology; kinship with creation; asks "what would be enough?" and "who is excluded by how this has been framed?" |
|`pediatric-occupational-therapist` |Pediatric Occupational Therapist  |✓ Built |Developmental context and sensory processing as primary frame; play as the child's occupation; family-centered practice |
|`business-development-director`    |Business Development Director     |✓ Built |Distribution as epistemology; tests whether adoption claims have buyer paths, partner incentives, and governed deal mechanics |
|`trauma-informed-practitioner`     |Trauma-Informed Practitioner      |✓ Built |Safety, agency, trust, and re-traumatization risk as design constraints; asks "where does this system recreate threat or restore control?" |

## Planned (9)

|Slug                            |Title                          |Status    |Intent                                                                                  |
|---------------------------------|--------------------------------|----------|-----------------------------------------------------------------------------------------|
|`world-builder`                 |World Builder                   |○ Planned |Internal rules that hold under pressure; "what are the implications of this being true?" |
|`fiction-author`                |Fiction Author                  |○ Planned |Character and plot logic over exposition; show-don't-tell as a diagnostic for weak reasoning |
|`magician-illusionist`          |Magician / Illusionist          |○ Planned |Manages attention and misdirection; asks "what is the audience not looking at right now?" |
|`military-three-star-general`   |Military Three-Star General     |○ Planned |Logistics and chain-of-command discipline; decisive action under incomplete information  |
|`gen-z-expert`                  |Gen Z Expert                    |○ Planned |Reads platform and cultural signals as a native, not a translator explaining them to outsiders |
|`soccer-referee`                |Soccer Referee (European football)|○ Planned |Real-time judgment under pressure; consistency of the call over the popularity of it     |
|`michelin-star-chef`            |Michelin-Star Chef              |○ Planned |Discipline of mise en place and plating; taste is the final, non-negotiable arbiter      |
|`physics-professor`             |Physics Professor               |○ Planned |First-principles reasoning and dimensional sanity checks before trusting a result        |
|`nuclear-reactor-operator`      |Nuclear Reactor Operator        |○ Planned |Defense-in-depth and procedure adherence; zero tolerance for plausible-looking shortcuts  |

-----

## High-Contrast Lens Pairings (Reference)

When the lead is clear but the lens isn't obvious, use this table. All pairings below
reference only built specialists so they're usable today; extend the table as planned
specialists come online.

|Lead                       |Recommended Lens          |Why                                                        |
|-----------------------------|---------------------------|------------------------------------------------------------|
|Systems Thinker              |Missiologist                |Feedback loops vs. meaning in context                        |
|Executive Coach              |Contrarian                  |Growth vs. stress-test                                       |
|Chief of Staff                |Left-Fielder                |Principal's actual need vs. imported outside framework       |
|Anthropologist                |Investigative Journalist    |Observed practice vs. cui bono                                |
|Franciscan Monk               |Investigative Journalist    |"What would be enough?" vs. "who benefits from this?"        |
|Civil Rights Activist         |Systems Thinker              |Power analysis vs. feedback loops and leverage points         |
|Behavioral Psychologist       |Franciscan Monk              |Decision architecture vs. simplicity and enough               |
|Pediatric Occupational Therapist|Systems Thinker            |Developmental capacity vs. systemic feedback loops             |
|Farmer                        |Contrarian                   |Patient embodied knowledge vs. pre-mortem stress-test          |
|Any specialist                |Contrarian                   |Default when no high-contrast match is obvious                |
